...
static size_t tournament_select(const int *fitness,
                                ...
                                uint64_t *state) {
    const size_t first = rng_index(state, population_size);
    const size_t second = rng_index(state, population_size);

    return fitness[first] >= fitness[second] ? first : second;
}

/* Each iteration uses its own RNG state and writes one disjoint child. */
void population_reproduce(const uint8_t *population,
    ...
    for (size_t i = 0; i < population_size; i++) {
        ...
        const size_t cut = 1 + rng_index(&state, bits_per_individual - 1);
        ...
        for (size_t bit = 0; bit < bits_per_individual; bit++) {
            ...
            if (rng_unit(&state) < mutation_rate) {
                child[bit] ^= 1u;
            }
        }
    }
